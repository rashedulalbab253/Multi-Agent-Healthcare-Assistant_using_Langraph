...
def build_router_prompt(note: Optional[str], has_image: bool = False) -> str:
    """
    ...
    """
    logger.info(f"Building router prompt with note: {note} and has_image: {has_image}")
    prompt = f"""You are a medical routing agent. Your task is to analyze the provided inputs
...
text: {note if note else "No text provided"}
image: {"An image has been provided" if has_image else "No image provided"}"""

    return prompt



def build_icd10_prompt(clinical_note: str) -> str:
    """
    ...
    Returns:
        str: The formatted prompt string.
    """
    prompt = f"""You are an expert clinical coder. Extract ICD-10 codes from the note below.
...
def build_image_analyzer_prompt(question: str = None) -> str:
    """
    ...
    Returns:
        str: The formatted prompt string.
    """
    prompt = f"""You are an expert radiologist and you are provided with an image of a medical condition.
...
def build_soap_generator_prompt(transcript: str) -> str:
    """
    ...
    Returns:
        str: The formatted prompt string.
    """
    prompt = f"""You are a clinical documentation assistant. Your task is to read medical 
...
You should return a JSON object with exactly the following fields:
